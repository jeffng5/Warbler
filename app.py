...
def users_show(user_id):
    ...
    liked_list= Likes.query.filter(Likes.user_id==Message.user_id)
    return render_template('users/show.html', user=user, messages=messages, liked_list=liked_list)
...
def users_followers(user_id):
    ...
    user = Follows.query.filter_by(user_being_followed_id= user_id)
...
def like_page(message_id):
    ...
    if g.user:
        ...
        post = Likes(user_id=g.user.id)
        db.session.add(post)
        db.session.commit()

    
        # messages = Likes.query.filter(Likes.user_id== Message.user_id).all()
    # quality = Message.query.filter_by(User.likes.message_id==Message.id, Message.user_id == user )
    
    # show = Message.query.filter_by(User.id==user and User.likes.user_id==user).all()
